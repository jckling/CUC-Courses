...
disp('掩密图像提取的水印'), disp(origin_str);
disp([name,'后提取的水印']),disp(handled_str);
disp(['正确检出率: ', num2str(diff_sum)]);
fprintf('\n');
% figure,subplot(1,2,1),imshow(img),title('掩密图像');subplot(1,2,2),imshow(handled_img),title(name);s

% 如果秘密信息是一维序列，比较长就变换一下
